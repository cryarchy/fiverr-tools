...
impl GigPage {
    pub fn seller_level_selector() -> Selector {
        Selector::new("#main-wrapper > .main-content .gig-page > .main .seller-card div:has(.rating-score) div:has(p) > p".to_owned())
    }

    pub fn get_seller_level(&self) -> Result<String, crate::Error> {
        let seller_level_selector = Self::seller_level_selector();
        let seller_level_els = self.tab.find_elements(&seller_level_selector, true)?;
